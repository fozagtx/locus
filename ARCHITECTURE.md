...
locus is a small distributed system disguised as a single-server app. Four moving parts, each doing one job.
...
- Has three tools via two MCP servers it owns:
  - `locus-memory.recall(query)` — pull relevant memories.
  - `locus-memory.write_memory(content, segment, importance, tier?)` — persist a durable fact.
  - `locus-spawn.spawn_agent(task, integrations[], name?)` — kick off an execution agent.
...
**`tools.ts`** — the `locus-memory` MCP server. `recall` and `write_memory`. Each call emits a `memoryEvents` row so you can watch it live in the dashboard.
...
Locus delegates all third-party integrations to [Composio](https://composio.dev/?utm_source=chris&utm_medium=youtube&utm_campaign=collab). One SDK, 1000+ toolkits, hosted auth.
...
4. Popup closes → frontend calls `POST /composio/refresh` → backend re-runs `registerComposioToolkits()` which iterates `connectedAccounts.list({ userIds: [locusUserId()] })` and registers each active toolkit as an `IntegrationModule` keyed by its slug.
...
```ts
await composio.create(locusUserId(), {
...
- `COMPOSIO_USER_ID` — optional; defaults to `locus-default` for single-tenant use.
...
- **Skills library omitted** — too Locus-specific; write your own prompts/policies in `server/*-agent.ts` system prompts.
