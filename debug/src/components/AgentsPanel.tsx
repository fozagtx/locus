...
const STATUS_CONFIG: Record<string, { dot: string; label: string; color: string }> = {
  spawned: { dot: "bg-amber-500", label: "Spawning", color: "text-amber-500" },
  running: { dot: "bg-sky-400", label: "Running", color: "text-sky-400" },
  completed: { dot: "bg-locus-brand", label: "Done", color: "text-locus-brand" },
  failed: { dot: "bg-rose-400", label: "Failed", color: "text-rose-400" },
  cancelled: { dot: "bg-locus-mute", label: "Cancelled", color: "text-locus-mute" },
};
              ...
              >
                ...
                {(agent.costUsd > 0 || totalTokens > 0) && (
                  <div className="flex items-center gap-3 text-[10px] mono mb-2">
                    {agent.costUsd > 0 && (
                      <span className="text-locus-brand font-semibold">
      ...
      <div className="p-5">
        <div
          className={`h-20 rounded-xl shimmer ${
            isDark ? "bg-locus-card/40" : "bg-locus-card"
          }`}
        />
      </div>
    );
      ...
      <div className="shrink-0 p-4 pb-2">
        <div
          className={`rounded-xl border shadow-lg px-4 py-3 ${
            isDark
              ? "bg-locus-card/90 border-sky-800/40"
        ...
        >
          ...
          >
            ...
            <span className={`ml-auto shrink-0 ${isDark ? "text-locus-mute" : "text-locus-mute"}`}>
              {requestOpen ? "▲" : "▼"}
            </span>
          </button>
          {requestOpen && (
        ...
        <div className="shrink-0 px-4 pb-2">
          <div
            className={`rounded-xl border px-4 py-2.5 ${
              isDark
                ? "bg-locus-card/60 border-locus-line/60"
              ...
              {agent.mcpServers.map((name) => (
                <span
                  ...
                  className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-[11px] font-medium ${
                    isDark
                      ? "bg-locus-card text-locus-ink"
                      : "bg-white text-locus-mute border border-locus-line"
                  }`}
                >
      ...
      {agent.status === "completed" && agent.result && (
        <div className="sticky bottom-0 p-4 pt-2">
          <div
            className={`rounded-xl border shadow-lg px-4 py-3 ${
              isDark
                ? "bg-locus-card/90 border-emerald-800/40"
          ...
          >
            <button
              ...
              className="flex items-center gap-2 w-full min-w-0"
            >
              <span className="w-2 h-2 rounded-full bg-locus-brand shrink-0" />
              <span
                className={`text-[10px] font-bold mono tracking-wider shrink-0 ${
                  isDark ? "text-locus-brand" : "text-locus-brand"
                }`}
              >
                RESPONSE
              </span>
      ...
      {agent.status === "failed" && agent.error && (
        <div className="sticky bottom-0 p-4 pt-2">
          <div
            className={`rounded-xl border shadow-lg px-4 py-3 ${
              isDark
                ? "bg-locus-card/90 border-rose-800/40"
