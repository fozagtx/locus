...
export function DashboardPanel({ isDark }: { isDark: boolean }) {
  ...
  const c = isDark
    ? {
        card: "bg-locus-card/60 border-locus-line",
        label: "text-locus-mute",
        value: "text-locus-ink",
        sub: "text-locus-mute",
        chart: "bg-locus-card/40 border-locus-line",
      }
    : {
        card: "bg-locus-card border-locus-line shadow-locus-emboss-strong",
        label: "text-locus-mute",
        value: "text-locus-ink",
        sub: "text-locus-mute",
        chart: "bg-locus-card border-locus-line shadow-locus-emboss-strong",
      };
      ...
      <div className="flex items-center justify-between">
        ...
        <div
          className={`flex items-center rounded-lg border text-xs ${
            isDark
              ? "border-locus-line bg-locus-card/50"
              : "border-locus-line bg-locus-card"
          }`}
        >
          {RANGES.map((r) => (
            <button
              ...
              className={`px-3 py-1.5 transition-colors ${
                range === r.id
                  ? isDark
                    ...
                    : "bg-white text-locus-ink font-medium shadow-sm"
                  : isDark
                    ? "text-locus-mute hover:text-locus-ink"
                    : "text-locus-mute hover:text-locus-mute"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div className={`rounded-xl border p-4 ${c.chart}`}>
          ...
          <div className="space-y-2">
            {(
              [
                ["completed", filtered.agents.completed, "bg-locus-brand"],
                ["failed", filtered.agents.failed, "bg-rose-500"],
                ["cancelled", filtered.agents.cancelled, "bg-locus-mute"],
              ] as const
            ...
            {filtered.agents.total === 0 && (
              <p className={`text-xs ${isDark ? "text-locus-mute" : "text-locus-mute"}`}>
          ...
          <div className="relative" ref={popRef}>
            <button
              ...
              className={`w-3.5 h-3.5 inline-flex items-center justify-center rounded-full border text-[9px] font-bold normal-case tracking-normal transition-colors ${
                isDark
                  ? "border-locus-line text-locus-mute hover:border-sky-400 hover:text-sky-400"
                  : "border-slate-400 text-locus-mute hover:border-sky-500 hover:text-sky-600"
              }`}
            >
              i
            </button>
...
function BarRow({
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <div className="flex-1 h-2 rounded-full overflow-hidden bg-white/70 shadow-[inset_0_1px_2px_rgba(0,37,97,0.08)]">
        <div
      ...
      {hoverIdx !== null && hovered && (
        <div
          className={`absolute pointer-events-none rounded-lg border px-3 py-2 shadow-lg text-xs z-10 ${
            isDark
              ? "bg-locus-card border-locus-line text-locus-ink"
              : "bg-white border-locus-line text-locus-ink"
          }`}
          style={{
            top: 4,
        ...
        >
          ...
          {keys.map((_, k) => (
            <div key={k} className="flex items-center gap-2 py-0.5">
              ...
              />
              <span className={isDark ? "text-locus-mute" : "text-locus-mute"}>
                {labels[k]}
              </span>
