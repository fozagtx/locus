...
Locus's integrations are provided by [Composio](https://composio.dev/?utm_source=chris&utm_medium=youtube&utm_campaign=collab), a tool-aggregator that exposes 1000+ third-party services (Gmail, GitHub, Slack, Notion, Linear, Google Drive, HubSpot, Salesforce, …) behind one API.
...
## How it hooks into Locus

Each connected Composio toolkit is registered in Locus's integration registry (`server/integrations/registry.ts`) keyed by its slug. When the dispatcher calls:
...
```ts
const session = await composio.create(locusUserId(), {
  toolkits: ["gmail"],
...
When you click Connect on one of those, Locus surfaces an amber banner explaining that you need to:
...
This is a one-time setup per toolkit (not per user) — all users of your Locus instance reuse the same auth config after that.

## Notes

- **Single-tenant by default.** All connections are keyed under `COMPOSIO_USER_ID` (defaults to `locus-default`). Override if you manage Composio sessions elsewhere and want Locus to share that user.
...
- **No tokens live in Locus.** Composio stores OAuth credentials on their side. Locus never sees them.
