...
export const CURATED_TOOLKITS: CuratedToolkit[] = [
  ...
  { slug: "exa", displayName: "Exa", authMode: "managed" },
];
...
export function locusUserId(): string {
  return process.env.COMPOSIO_USER_ID ?? "locus-default";
}
...
export async function listConnectedToolkits(): Promise<ConnectedToolkit[]> {
  ...
  if (!composio) return [];
  try {
    const resp = await composio.connectedAccounts.list({ userIds: [locusUserId()] });
...
export async function authorizeToolkit(
  ...
  const conn = await composio.connectedAccounts.initiate(locusUserId(), authConfigId, {
...
export async function ensureTrigger(
  ...
  try {
    const resp = await composio.triggers.create(locusUserId(), triggerSlug, {
      connectedAccountId,
    });
