...
export function createSelfMcp() {
  return createSdkMcpServer({
    ...
    tools: [
      tool(
        "get_config",
        "Return Locus's runtime configuration: which Claude model it's using, the user's timezone, the current local time, which integrations are loaded, and basic env info. Use when the user asks 'what model are you?', 'what time is it?', 'what timezone am I in?', or anything about the agent itself.",
        {},
        async () => {
          ...
          const config = {
            ...
            envDefault: process.env.LOCUS_MODEL ?? "claude-sonnet-4-6",
            ...
            spectrumEnabled: process.env.SPECTRUM_ENABLED === "1" && Boolean(process.env.SPECTRUM_PROJECT_ID),
          };
          return {
      ...
      tool(
        "set_timezone",
        `Save the user's timezone so Locus can reason about deadlines, "today", "9am tomorrow", and other local-time references correctly. Accepts an IANA timezone ID (e.g. "America/Chicago", "Europe/London") or a friendly alias ("central", "PT", "Dallas", "Tokyo", "UTC", etc.).
