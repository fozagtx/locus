...
// Bring whatever the user put in LOCUS_USER_PHONE to E.164 (+1XXXXXXXXXX).
...
async function dispatchProactiveNotice(summary: string): Promise<void> {
  const raw = process.env.LOCUS_USER_PHONE;
  if (!raw) {
    console.warn("[proactive] LOCUS_USER_PHONE not set; skipping dispatch");
    return;
  }
  const phone = normalizeProactivePhone(raw);
  if (!phone) {
    console.warn(
      `[proactive] LOCUS_USER_PHONE=${JSON.stringify(raw)} doesn't look like a valid phone number; skipping dispatch`,
    );
    return;
  }
...
export async function ensureProactiveWatcher(publicUrl: string): Promise<void> {
  ...
  if (!process.env.LOCUS_USER_PHONE) {
    console.warn(
      "[proactive] LOCUS_USER_PHONE not set; webhook will register but notices won't dispatch",
    );
  }
  try {
