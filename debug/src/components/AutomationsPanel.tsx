...
const STATUS_COLOR: Record<string, { dot: string; text: string }> = {
  ...
  completed: { dot: "bg-locus-brand", text: "text-locus-brand" },
...
export function AutomationsPanel({ isDark }: { isDark: boolean }) {
  ...
  const mutedText = isDark ? "text-locus-mute" : "text-locus-mute";
    ...
    <div className="flex flex-col h-full -m-5 fade-in">
      ...
      <div
        className={`shrink-0 border-b px-5 py-3 space-y-2 ${
          isDark ? "border-locus-line/50" : "border-locus-line"
        }`}
      >
        <div>
          <span
          ...
          <span
            className={`text-xs ${isDark ? "text-locus-mute" : "text-locus-mute"}`}
          >
            {auto.task}
          </span>
        </div>
