      ...
      try {
        ...
        if (phones.length === 1) {
          ...
          const { pickedNumber } = await prompts({
            type: "select",
            name: "pickedNumber",
            message: "You have multiple Sendblue numbers — which one should Locus reply from?",
...
`);
  ...
  const inferredCurrent = existingVoyage
    ...
    : existingOpenai
      ...
      : "local";
  console.log(`
Locus's recall() searches your stored memories by semantic similarity. Pick
  ...
  const { embeddingProvider } = await prompts(
    {
      ...
      message: "Which embedding provider should locus use?",
      choices: [
...
`);
  ...
  if (tunnelChoice === "ngrok-domain") {
    const { NGROK_DOMAIN } = await prompts({
      type: "text",
      name: "NGROK_DOMAIN",
      message: "Your ngrok reserved domain (e.g. locus.ngrok.app, no https://):",
    ...
    const { PUBLIC_URL } = await prompts({
      type: "text",
      name: "PUBLIC_URL",
      message: "Your stable public URL (e.g. https://locus.mydomain.com):",
