...
export function prettyToolName(raw?: string | null): string {
  ...
  if (server && action) {
    ...
    if (normalize(server).startsWith("locus-")) return prettyAction;
...
// Locus-tool SVG icons (stroke-based, tint via currentColor)
const LOCUS_ICONS: Record<string, (s: number) => ReactNode> = {
  recall: (s) => (
  ...
  write_memory: (s) => LOCUS_ICONS.recall(s),
  save_draft: (s) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 20h9" />
  ...
  list_automations: (s) => LOCUS_ICONS.create_automation(s),
  toggle_automation: (s) => LOCUS_ICONS.create_automation(s),
  delete_automation: (s) => LOCUS_ICONS.create_automation(s),
  list_drafts: (s) => LOCUS_ICONS.save_draft(s),
  reject_draft: (s) => (
    <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <circle cx="12" cy="12" r="10" />
      <path d="m15 9-6 6" />
...
function getLocusToolIcon(raw?: string | null): ((s: number) => ReactNode) | null {
  if (!raw) return null;
  const action = raw.split("__").pop() ?? raw;
  return LOCUS_ICONS[action] ?? null;
}
...
export function IntegrationLogo({
  ...
  const locusIcon = getLocusToolIcon(raw);
